...
const orbitingObject = new THREE.Mesh(orbitingObjectGeometry, orbitingObjectMaterial);
...
const ringGeometry = new THREE.RingGeometry(orbitRadius - 0.05, orbitRadius + 0.05, 18);
...
/**
 * Animate the scene
 * - Updates the rotation of the main planet.
 * - Moves the smaller planet along the orbit path.
 * - Continuously re-renders the scene for a smooth animation.
 */
function animate() {
    ...
    orbitingObject.position.z = planet.position.z + orbitRadius * Math.sin(angle) * Math.cos(orbitTilt);
    orbitingObject.position.y = orbitRadius * Math.sin(orbitTilt) * Math.sin(angle);
